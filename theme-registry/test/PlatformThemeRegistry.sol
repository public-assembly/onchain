...
contract PlatformThemeRegistryTest is Test {
    // Setup variables
    address payable public constant DEFAULT_OWNER_ADDRESS = payable(address(0x999));
    address payable public constant NON_OWNER_1 = payable(address(0x888));
    address payable public constant NON_OWNER_2 = payable(address(0x777));
    ...
    function getInitHash() public pure returns (bytes32) {
        bytes memory bytecode = type(PlatformThemeRegistry).creationCode;
    ...
    function test_deploy() public {
        vm.startPrank(DEFAULT_OWNER_ADDRESS);
        require(
            keccak256(bytes(platformThemeRegistry.contractDocumentation())) == keccak256(bytes(testContractDocs_1)),
            "theme registry not deployed correctly"
        );
    ...
    function test_newContractDocs() public {
        ...
        platformThemeRegistry.setContractDocumentation(testContractDocs_2);
        require(
            keccak256(bytes(platformThemeRegistry.contractDocumentation())) == keccak256(bytes(testContractDocs_2)),
            "theme registry not updated correctly"
        );
    }
    ...
    function test_newPlatformIndex() public {
        ...
        require(
            platformThemeRegistry.getRole(1, DEFAULT_OWNER_ADDRESS) == PlatformThemeRegistry.Roles.ADMIN,
            "roles not working"
        );
        require(
            keccak256(bytes(platformThemeRegistry.getPlatformTheme(1))) == keccak256(bytes(testThemeInfo_1)),
            "themes not working"
        );
    }
    ...
    function test_setPlatformTheme() public {
        ...
        require(
            keccak256(bytes(platformThemeRegistry.getPlatformTheme(1))) == keccak256(bytes(testThemeInfo_2)),
            "themes not working"
        );
    }
    ...
    function test_grantAndRevokeRoles() public {
        ...
        require(
            platformThemeRegistry.getRole(1, NON_OWNER_1) == PlatformThemeRegistry.Roles.MANAGER, "roles not working"
        );
        vm.stopPrank();
        vm.startPrank(NON_OWNER_1);
        platformThemeRegistry.setPlatformTheme(1, testThemeInfo_2);
        ...
        require(
            platformThemeRegistry.getRole(1, NON_OWNER_2) == PlatformThemeRegistry.Roles.MANAGER, "roles not working"
        );
        address[] memory accounts = new address[](1);
        accounts[0] = NON_OWNER_2;
        platformThemeRegistry.revokeRoles(1, accounts);
        require(
            platformThemeRegistry.getRole(1, NON_OWNER_2) == PlatformThemeRegistry.Roles.NO_ROLE, "roles not working"
        );
    }
}
